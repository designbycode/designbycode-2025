...
class ImageConverter extends Component
{
    ...
    public function downloadConverted(): void
    {
        if (!$this->convertedImagePath) {
            $this->addError('download', 'No converted file available for download.');
            return;
        }
        ...
        // Create a simple download link using storage URL
        $publicUrl = Storage::url($this->convertedImagePath);
